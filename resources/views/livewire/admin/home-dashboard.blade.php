    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
        <div class="lg:col-span-3 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-4">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-4">
                <h3 class="text-sm font-semibold text-gray-900 dark:text-white">Permit Status Overview ({{ ucfirst($range) }})</h3>
        ...
        <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-4">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-sm font-semibold text-gray-900 dark:text-white">Permits by Status</h3>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-4">
            ...
            @php
                ...
                $statusMeta = [
                    0 => ['label' => 'Scheduled',   'color' => 'text-blue-600 dark:text-blue-400'],
                    1 => ['label' => 'In Progress',  'color' => 'text-amber-600 dark:text-amber-400'],
                    2 => ['label' => 'Completed',    'color' => 'text-emerald-600 dark:text-emerald-400'],
                    3 => ['label' => 'Cancelled',    'color' => 'text-red-600 dark:text-red-400'],
                    4 => ['label' => 'On Hold',      'color' => 'text-purple-600 dark:text-purple-400'],
                    5 => ['label' => 'Returned',     'color' => 'text-orange-600 dark:text-orange-400'],
                    6 => ['label' => 'Lapsed',       'color' => 'text-gray-500 dark:text-gray-400'],
                    7 => ['label' => 'Resolved',     'color' => 'text-cyan-600 dark:text-cyan-400'],
                ];
            @endphp

            @if($selected)
                <div class="text-xs text-gray-500 dark:text-gray-400 mb-3">
                    {{ \Carbon\Carbon::parse($selected['date'])->format('F j, Y') }}
                </div>

                <div class="space-y-2">
                    <div class="flex items-center justify-between text-sm pb-2 border-b border-gray-100 dark:border-gray-700">
                        <div class="font-medium text-gray-700 dark:text-gray-200">Total</div>
                        <div class="font-bold text-gray-900 dark:text-white">{{ (int) ($selectedCounts['total'] ?? 0) }}</div>
                    </div>

                    @foreach($statusMeta as $code => $meta)
                        @php $count = (int) ($selectedCounts[$code] ?? 0); @endphp
                        <div class="flex items-center justify-between text-sm">
                            <div class="text-gray-600 dark:text-gray-300">{{ $meta['label'] }}</div>
                            <div class="font-semibold {{ $count > 0 ? $meta['color'] : 'text-gray-400 dark:text-gray-600' }}">
                                {{ $count }}
                            </div>
                        </div>
                    @endforeach
