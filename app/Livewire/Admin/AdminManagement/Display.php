...
class Display extends Component
{
    ...
    protected function baseQuery(): Builder
    {
        ...
        if ($this->statusFilter === 'disabled') {
            $admins->where('is_disabled', true);
        } elseif ($this->statusFilter === 'enabled') {
            $admins->where('is_disabled', false);
        }

        if ($this->dateFrom || $this->dateTo) {
            if ($this->dateFrom && $this->dateTo) {
                $admins->whereBetween('created_at', [$this->dateFrom . ' 00:00:00', $this->dateTo . ' 23:59:59']);
            } elseif ($this->dateFrom) {
                $admins->whereDate('created_at', '>=', $this->dateFrom);
            } elseif ($this->dateTo) {
                $admins->whereDate('created_at', '<=', $this->dateTo);
            }
        }
